...
function loadPriceDB() {
  ...
  const possiblePaths = [
    ...
    path.join(process.cwd(), "prices", "price_db.json"),
    "/var/task/app/prices/price_db.json",
  ];
  for (const p of possiblePaths) {
    console.log("[price-matcher] Trying path:", p, "exists:", fs.existsSync(p));
    if (fs.existsSync(p)) {
      priceDB = JSON.parse(fs.readFileSync(p, "utf-8"));
      console.log("[price-matcher] Loaded", priceDB.length, "positions from", p);
      return priceDB;
    }
  }
  console.error("[price-matcher] price_db.json NOT FOUND in any path");
  priceDB = [];
  return priceDB;
}
